...
{
    public partial class AccountingForConf : Form
    {
        ...
        string idName = "";
        string name_person = "";
        public AccountingForConf(Scientific_work parent, NpgsqlConnection connection)
        {
            InitializeComponent();
            conn = connection;
        }
        ...
        private void AddButton_Click(object sender, EventArgs e)
        {
            ...
            sql = $"SELECT * FROM Accounting_for_conf('{config}')";
        ...
        private void DeleteButton_Click(object sender, EventArgs e)
        {
            foreach (DataGridViewRow row in dataGridView1.SelectedRows)
            {
                ...
                sql_ = $"DELETE FROM {tableName} WHERE {idName} = {p1} AND Id_cnf = {p2};";
                var cmd = new NpgsqlCommand(sql_, conn);
                cmd.ExecuteNonQuery();
